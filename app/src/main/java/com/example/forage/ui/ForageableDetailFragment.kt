...
class ForageableDetailFragment : Fragment() {
    ...
    // ForageableViewModelFactory. The factory should take an instance
    // of the Database retrieved from BaseApplication
    private val viewModel: ForageableViewModel by activityViewModels {
        ForageableViewModelFactory(
            (activity?.application as BaseApplication).database.getForageableDao()
        )
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getForageableById(id).observe(this.viewLifecycleOwner, { it ->
            it.let {
                forageable = it
                bindForageable()
            }
